...
export const InfographicInputSchema = z.object({
  ...
  narrativeFocus: z.enum(['data-heavy', 'narrative-driven', 'action-oriented']).default('data-heavy'),
});
...
export const BrandStyleSchema = z.object({
  ...
  accentColor: z.string().describe("A contrasting color for highlighting critical insights"),
  fontMood: z.enum(['corporate', 'modern-sans', 'playful', 'monospaced']),
  borderRadius: z.string().describe("Tailwind-style or CSS value (e.g., '0.5rem')"),
  layoutDensity: z.enum(['compact', 'airy', 'spacious']).describe("Controls whitespace and padding")
});
...
export const InfographicContentSchema = z.object({
  ...
  summary: z.string().describe("The 'Too Long; Didn't Read' summary for the header"),
  ...
  metadata: z.object({
    ...
    reasoning: z.string().describe("Briefly explain why these sections were chosen for this data")
  })
});